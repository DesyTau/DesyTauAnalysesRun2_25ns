...
root -l -b -q `printf 'Plot_lept_mutau_NNNTuples.C("bcsv_1","bcsv_1",10,0.7,1.,"%s","(mt_1<50)*","Events",-1,"%s","%s",2017,%s,false,false,%s,%s)' $WEIGHT $PATH_TO_TUPLES $PATH_FOR_OUTPUT $APPLY_FF $SHOW_SIGNAL $COMPARE_CP` 
root -l -b -q `printf 'Plot_lept_mutau_NNNTuples.C("bcsv_2","bcsv_2",10,0.7,1.,"%s","(mt_1<50)*","Events",-1,"%s","%s",2017,%s,false,false,%s,%s)' $WEIGHT $PATH_TO_TUPLES $PATH_FOR_OUTPUT $APPLY_FF $SHOW_SIGNAL $COMPARE_CP` 
