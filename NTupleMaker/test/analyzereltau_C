...
//	CutList.push_back("Iso lt 0.5");
	...
	CutList.push_back("dR lt 3");
	CutList.push_back("Extra");
...
*/
	...
	TH1D * histTop ;
	if (isTT) histTop = (TH1D*)filein->Get("eltau/histTopPt");
...
//		cout<<" e " <<jentry<<" from "<<nentries<<"  weights "<<all_weights<<" same ? "<< gen_weight*trig_weight*pu_weight*top_weight*LSF_weight*TFR_weight <<" PU "<<pu_weight<<" top "<<top_weight<<" LSF "<<LSF_weight<<" TFR "<<TFR_weight<<" gen "<<gen_weight<<" trig  "<<trig_weight<<endl;
		...
		if (el_relIso[0] > 0.3) continue;	

	
//		if ( fabs(el_dz[electron_index]) > 0.025 && !InvertLeptonIso) continue;
//		if ( fabs(el_dz[electron_index]) < 0.025 && InvertLeptonIso) continue;

//		if ( fabs(el_dxy[electron_index]) > 0.025 && !InvertLeptonIso) continue;
//		if ( fabs(el_dxy[electron_index]) < 0.025 && InvertLeptonIso) continue;
