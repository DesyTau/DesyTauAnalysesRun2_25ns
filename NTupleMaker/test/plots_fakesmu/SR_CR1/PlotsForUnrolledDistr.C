...
void PlotsForUnrolledDistr(TString directory = "",
	  TString suffix = "",
	  TString File = "Templ_CutFlowUnWFakeRateJet_17_0_35invfb_mt_C1N2_SR_CR1.root",
	  TString variable = "",
	  TString Suffix = "",
	  TString xtitle = "",
...

{
	  ...
	  vars_.push_back("CutFlowUnWFakeRateJet_17_0");
  //ModTDRStyle();
...
for (int svar=0;svar<vars_.size();++svar)
{
    ...
    TH1D * histSignal2 = (TH1D*)file->Get("1D_stau-stau_left_100_LSP1_B_"+Variable);
...
//{for (int iB=1; iB<=nBins; ++iB) {TTcl->SetBinContent(iB,0);}}
  ...
  std::cout << "Rest   : " << VV->GetSumOfWeights() << " : "  <<" GetNbinsX= " << VV->GetNbinsX() << "  Purity  "<<VV->GetSumOfWeights()/allbkg->GetSumOfWeights()<< std::endl;
...
///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (int nB=1;nB<WJ->GetNbinsX();++nB) cout<<" nB "<<nB<<" Label "<<BinLabels[nB-1]<<"  "<<WJ->GetBinContent(nB)/WJ->GetSumOfWeights()<<endl;
