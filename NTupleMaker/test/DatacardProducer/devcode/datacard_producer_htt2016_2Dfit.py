...
def applyRelaxedSelection(categoryName):
	if categoryName in ["1jet_high","vbf_low","vbf_high","vbf","1jet"]:
		return True
	else: 
		return False

#def setRelaxedSelection():
	...
	for ckey, cvalue in cuts.iteritems():
		...
		for hkey, hvalue in histos.iteritems():
			...
			if (datacardFor2Dfit==True):
				...
				bins_2Dfit = {
				"0jet": {"varX":"pt_2","binX":[20,30,40,50,13000], "axistit":"tau pT"},
				"1jet": {"varX":"pt_tt","binX":[0,60,90,120,160,13000], "axistit":"Higgs pT"},
				"vbf": {"varX":"mjj","binX":[300,600,900,1200,1600,13000], "axistit":"mjj"}
				}

				varX = bins_2Dfit[ckey]["varX"]
				binsX = array('d',bins_2Dfit[ckey]["binX"])
				nbinsX = len(binsX)-1
				titX = bins_2Dfit[ckey]["axistit"]
				varY = var
				nbinsY = nbins
				minY = xmin
				maxY = xmax
				...
				print "used for hdummy: nbinsX = ", nbinsX, " |  binsX = " , binsX, " | nbinsY = ", nbinsY, " | minY ", minY , " | maxY ", maxY
				hdummy = TH2F("hdummy","hdummy"+axisTit,nbinsX,binsX,nbinsY,minY,maxY)
				var = varY+":"+varX
