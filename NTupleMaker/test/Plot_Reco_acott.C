...
void Plot_Reco_acott(
		 ...
		 TString Suffix = "CPMuTau_",        // for name of pdf
  ...
  TString Cut="";
  /*
  if(firstDecay==0)Cut+= "(tau_decay_mode_1==0)";
  if(firstDecay==-1)Cut+= "(tau_decay_mode_1<=6||tau_decay_mode_1==8||tau_decay_mode_1==9)";
  if(firstDecay==8)Cut+= "(tau_decay_mode_1==8)";
  if(firstDecay==9)Cut+= "(tau_decay_mode_1==9)";
  if(firstDecay==1)Cut+= "(tau_decay_mode_1>=1&&tau_decay_mode_1<=2)";
  if(firstDecay==2)Cut+= "(tau_decay_mode_1>=4&&tau_decay_mode_1<=5)";
  Cut+= "&&";
  */

  //TOFIX: macro completed only for mutau channel
  if(firstDecay!=0){
    cout << "ERROR: macro made only for mutau channel! Only the first decay mode = 0 is currently accepted" << endl;
    return;
  }


  if(secondDecay==0)Cut+= "(tau_decay_mode_2==0)";
  if(secondDecay==-1)Cut+= "(tau_decay_mode_2<=6||tau_decay_mode_2==8||tau_decay_mode_2==9)";
  //if(secondDecay==8)Cut+= "(tau_decay_mode_2==8)";
  //if(secondDecay==9)Cut+= "(tau_decay_mode_2==9)";
  if(secondDecay==1)Cut+= "(tau_decay_mode_2>=1&&tau_decay_mode_2<=2)";
  if(secondDecay==2)Cut+= "(tau_decay_mode_2==10)";
  ...
  for (int i=0; i<nSamples; ++i) {
    ...
    tree->Draw(Variable+">>"+histName,Cut+Cuts);
  ...
  */
  ...
  canv1->Update();
  ggH->SetTitle("CP measurement in #tau_h#tau_h");
  ggA->SetTitle("CP measurement in #tau_h#tau_h");
  xtitle+= " channel ";
