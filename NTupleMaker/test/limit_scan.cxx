...
namespace{
  int Nsmooth = 0; // Number of times to smooth TH2D
...
int main(int argc, char *argv[]){
 ...
 //   else 
  ...
  if(model=="stau") xparticle = "stau";
  if(model=="C1N2") xparticle = "chargino";
  if(model=="C1C1") xparticle = "chargino";
  if(model=="T6ttWW") {
    xparticle = "sbottom";
    yparticle = "chargino";
  }
...
TGraph DrawContours(TGraph2D &g2, int color, int style,
  ...
  if(Nsmooth>0){
	...
	vx.push_back(x);
	vy.push_back(y);
	if(x-y>glu_lsp+25) vz.push_back(z);
