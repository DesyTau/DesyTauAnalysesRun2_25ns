...
if (!CutBasedTauId){
   ...
   //isoTau = analysisTree.tau_chargedIsoPtSum[tIndex];
	...
	if (isLoose)   ta_isLoose = 1;
	if (!isLoose)   ta_isLoose = 0;
...
if (isTight)
	...
	if (isTight) ta_isTight = 1;
	if (!isTight) ta_isTight = 0;
      ...
      bool isGenLeptonPromptMuMatched = false;
      bool isGenLeptonPromptElMatched = false;
      if (!isData){
	...
	for (unsigned int gt = 0 ; gt < analysisTree.gentau_count; ++gt){
	 ...
	 // genTauV.SetXYZT(0.,0.,0.,0.);
	  ...
	  double Drm=deltaR(analysisTree.tau_eta[tau_index],analysisTree.tau_phi[tau_index],
		...
		if (Drl < 0.2 && genLepV.Pt() > 8.  && abs(analysisTree.genparticles_pdgid[igen])==11 && analysisTree.genparticles_isPrompt[igen] > 0.5) isGenLeptonPromptElMatched = true;
		if (Drl < 0.2 && genLepV.Pt() > 8.  && abs(analysisTree.genparticles_pdgid[igen])==13 && analysisTree.genparticles_isPrompt[igen] > 0.5) isGenLeptonPromptMuMatched = true;
	...
	genLeptonMatched = isGenLeptonMatched;
	genLeptonPromptElMatched = isGenLeptonPromptElMatched;
	genLeptonPromptMuMatched = isGenLeptonPromptMuMatched;
