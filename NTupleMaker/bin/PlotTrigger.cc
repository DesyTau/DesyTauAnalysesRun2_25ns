...
void PlotCrossTrigger(TString WP,
  ...
  TString cutsBase("ngentaus==2&&deltaR>0.5&&genTauDecay>=0&&genTauDecay<8&&genTauFoundReco&&tauNewDM>0.5&&tauby"+WP+"DeepTau2017v2p1VSjet&&tauPt>20&&TMath::Abs(tauEta)<2.1");
 
  cutsBase = cutsBase + "&&" + etaCut[iEta] + "&&" + WPCut[iEta];

  TString cutsMuTau = ("&&genTau1Decay==8&&genTau1FoundReco&&tau1Pt>28&&tau1Iso<0.1&&tau1SingleMuon&&TMath::Abs(tau1Eta)<2.1");
...
void PlotTriggerWP(TString WP,
  ...
  leg->Draw();
  canv->Update();
  canv->Print("SingleTauTrigger_"+Sample+DMLabel+"_wp.png");
  ...
  leg1->Draw();
  canv1->Update();
  canv1->Print("SingleTauTrigger_"+SampleLabel+DMLabel+"_eta.png");
...
int main(int argc, char * argv[]) {
  ...
  TString embedded(argv[1]);
  TString era(argv[2]);

  /*  
  int nBins1 = 20;
  float bins1[21] = {20,25,30,35,40,45,50,60,70,80,90,100,120,140,160,180,200,225,250,300,400,2000};

  int nBins = 13;
  float bins[14] = {60,80,100,120,140,160,180,200,225,250,300,400,700,2000};
  */
