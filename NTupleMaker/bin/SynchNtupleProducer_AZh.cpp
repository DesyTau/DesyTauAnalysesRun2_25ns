...
int main(int argc, char * argv[]) {
  ...
  TH1D * inputEventsH = new TH1D("inputEventsH","",1,-0.5,0.5);
    ...
    SynTree->Branch("m_ll",&m_ll,"m_ll/F");
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) {
	...
	cout << "Filter " << SingleMuonFilterName << " not found " << endl;
        ...
        for (unsigned int im = 0; im<analysisTree.muon_count; ++im) {
            ...
            //Isolation DR cone size 0.4 for muons
        ...
        //Final state selection: EEMuTau
        if(idEles.size() != 2) continue;
        if(idMuons.size() != 1) continue;
        if(idTaus.size() != 1) continue;
        ...
        for(unsigned int im=0;im<goodMuons.size();++im)
        {
            ...
            //Muon isolation calculation (No need for Synchronization)
            /*
            float absIso = analysisTree.muon_r04_sumChargedHadronPt[indexTemp1];
            float neutralIso = analysisTree.muon_r04_sumNeutralHadronEt[indexTemp1] + analysisTree.muon_r04_sumPhotonEt[indexTemp1] - 0.5*analysisTree.muon_r04_sumPUPt[indexTemp1];
            neutralIso = TMath::Max(float(0),neutralIso);
            absIso += neutralIso;
            float relIso = absIso/analysisTree.muon_pt[indexTemp1];
            if (relIso<0.15) continue; // to suppress background, harder than the baseline muon selection
            */
            for(unsigned int it=0;it<goodTaus.size();++it)
            {
                int indexTemp2 = goodTaus[it];
                float q2 = analysisTree.tau_charge[indexTemp2];
                if (q1*q2 > 0) continue;
        ...
        if(foundHOS==false) continue;
        //std::cout << "index muon 1: " << indexMuon1 << std::endl;
        //std::cout << "index tau 1: " << indexTau1 << std::endl;
        ...
        IdRawMva_1 = analysisTree.electron_mva_value_noIso_Fall17_v1[indexElectron1];
        ...
        IdRawMva_2 = analysisTree.electron_mva_value_noIso_Fall17_v1[indexElectron2];
        
        pt_3 = analysisTree.muon_pt[indexMuon1];
        phi_3 = analysisTree.muon_phi[indexMuon1];
        eta_3 = analysisTree.muon_eta[indexMuon1];
        
        float absIso_3 = analysisTree.muon_r04_sumChargedHadronPt[indexMuon1];
        float neutralIso_3 = analysisTree.muon_r04_sumNeutralHadronEt[indexMuon1] + analysisTree.muon_r04_sumPhotonEt[indexMuon1] - 0.5*analysisTree.muon_r04_sumPUPt[indexMuon1];
        neutralIso_3 = TMath::Max(float(0),neutralIso_3);
        absIso_3 += neutralIso_3;
        iso_3 = absIso_3/analysisTree.muon_pt[indexMuon1];
        
        pt_4 = analysisTree.tau_pt[indexTau1];
        phi_4 = analysisTree.tau_phi[indexTau1];
        eta_4 = analysisTree.tau_eta[indexTau1];
        iso_4 = analysisTree.tau_byDeepTau2017v2p1VSjetraw[indexTau1];
        ...
        if (!isData)
        {
            for (unsigned int igen=0; igen < analysisTree.genparticles_count; ++igen)
            {
                ...
                if (isAnyType)
                {
                    ...
                    float deltaR_3 = deltaR(analysisTree.muon_eta[indexMuon1],analysisTree.muon_phi[indexMuon1],etaGen,phiGen);
                    if (deltaR_3<minDR_3)
                    {
                        minDR_3 = deltaR_3;
                    ...
                    float deltaR_4 = deltaR(analysisTree.tau_eta[indexTau1],analysisTree.tau_phi[indexTau1],etaGen,phiGen);
                    if (deltaR_4<minDR_4)
                    {
                        minDR_4 = deltaR_4;
            ...
            for (unsigned int igen=0; igen < analysisTree.gentau_count; ++igen)
            {
                if (analysisTree.gentau_visibleNoLep_pt[igen]>15.)
                {
                    ...
                    float deltaR_3 = deltaR(analysisTree.muon_eta[indexMuon1],analysisTree.muon_phi[indexMuon1],analysisTree.gentau_visibleNoLep_eta[igen],analysisTree.gentau_visibleNoLep_phi[igen]);
                    if (deltaR_3<minDR_3)
                    {
                        minDR_3 = deltaR_3;
                        gen_match_3 = 5;
                    }
                    float deltaR_4 = deltaR(analysisTree.tau_eta[indexTau1],analysisTree.tau_phi[indexTau1],analysisTree.gentau_visibleNoLep_eta[igen],analysisTree.gentau_visibleNoLep_phi[igen]);
                    if (deltaR_4<minDR_4)
                    {
                        minDR_4 = deltaR_4;
                        gen_match_4 = 5;
        ...
        pfmt_3=mT(muon1,metLV);

        TLorentzVector tau2;
        tau2.SetXYZM(analysisTree.tau_px[indexTau1],analysisTree.tau_py[indexTau1],analysisTree.tau_pz[indexTau1],analysisTree.tau_mass[indexTau1]);
        pfmt_4=mT(tau2,metLV);

        m_vis = (muon1+tau2).M();
        pt_tt = (muon1+tau2).Pt();
        m_ll = (electron1+electron2).M();
        Z_Pt = (electron1+electron2).Pt();
        
        if ( m_ll < 60 || m_ll > 120)
            continue;
        Mass = (electron1+electron2+muon1+tau2).M();
