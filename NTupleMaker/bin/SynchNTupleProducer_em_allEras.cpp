...
int main(int argc, char * argv[]) {
   ...
   // first argument - config file
    ...
    // tracking weights from correction WS
	...
	// numberOfEntries = 10000;
       ...
       badMuonFilter_ = metFiltersPasses(analysisTree,badMuonFlag);
	    ...
	    chargedHadronTrackResolutionFilter_ = metFiltersPasses(analysisTree,chargedHadronTrackResolutionFlag);
            ...
            for (unsigned int ie = 0; ie<analysisTree.electron_count; ++ie) {
               ...
               if (is2016) 
                  {
                     electronMvaId = analysisTree.electron_mva_wp80_general_Spring16_v1[ie]>0.5; 
                  }
               else {
       ...
       if (muons.size()==0) continue;          
            ...
            if (is2016) metFilters_ = metFilters_ && badMuonFilter_ && duplicateMuonFilter_ &&  badChargedCandidateFilter_ && badPFMuonFilter_;              
            else metFilters_ = metFilters_;
            if (is2016 && badMuonFilter_ < 0.5) continue;
            if (is2016 && duplicateMuonFilter_ < 0.5) continue; 
       ...
       njets_jecUncRelativeSampleUp   = 0;
	    ...
	    metLV.SetXYZT(met_x,met_y,0.,met);
          ...
          if (!is2016 && jetPt < 50 && absJetEta > 2.65 && absJetEta < 3.139) continue;
