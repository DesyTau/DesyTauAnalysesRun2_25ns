...
void FakeRate(float pt, float * mean, float * error, int &iso) {
  ...
  // up-to-date
  // if (pt<150) {
  //   mean[0]  = 0.095; error[0] = 0.025;
  //   mean[1]  = 0.062; error[1] = 0.023;
  //   mean[2]  = 0.039; error[2] = 0.015;
  //   iso = 0;
  // }
  // else if (pt>=150&&pt<200) {
  //   mean[0]  = 0.091; error[0] = 0.023;
  //   mean[1]  = 0.069; error[1] = 0.020;
  //   mean[2]  = 0.044; error[2] = 0.015;
  //   iso = 1;
  // }
  // else {
  //   mean[0]  = 0.072; error[0] = 0.027;
  //   mean[1]  = 0.032; error[1] = 0.021;
  //   mean[2]  = 0.021; error[2] = 0.015;
  //   iso = 2;
  // }

  //  if (pt<150) {
  //    mean[0]  = 0.081; error[0] = 0.007;
  //    mean[1]  = 0.051; error[1] = 0.006;
  //    mean[2]  = 0.032; error[2] = 0.004;
  //    iso = 0;
  //  }
  //  else if (pt>=150&&pt<200) {
  //    mean[0]  = 0.092; error[0] = 0.016;
  //    mean[1]  = 0.068; error[1] = 0.012;
  //    mean[2]  = 0.046; error[2] = 0.014;
  //    iso = 1;
  //  }
  //  else {
  //    mean[0]  = 0.082; error[0] = 0.052;
  //    mean[1]  = 0.047; error[1] = 0.047;
  //    mean[2]  = 0.030; error[2] = 0.030;
  //    iso = 2;
  //  }

  // adjusted
  // if (pt<150) {
  //   mean[0]  = 0.083; error[0] = 0.009;
  //   mean[1]  = 0.054; error[1] = 0.007;
  //   mean[2]  = 0.035; error[2] = 0.004;
  //   iso = 0;
  // }
  // else if (pt>=150&&pt<200) {
  //   mean[0]  = 0.100; error[0] = 0.013;
  //   mean[1]  = 0.062; error[1] = 0.008;
  //   mean[2]  = 0.040; error[2] = 0.008;
  //   iso = 1;
  // }
  // else {
  //   mean[0]  = 0.060; error[0] = 0.020;
  //   mean[1]  = 0.024; error[1] = 0.024;
  //   mean[2]  = 0.014; error[2] = 0.014;
  //   iso = 2;
  // }

  // January 11th
  // if (pt<150) {
  //   mean[0]  = 0.0743; error[0] = 0.0055;
  //   mean[1]  = 0.0478; error[1] = 0.0045;
  //   mean[2]  = 0.0296; error[2] = 0.0036;
  //   iso = 0;
  // }
  // else if (pt>=150&&pt<200) {
  //   mean[0]  = 0.0874; error[0] = 0.0111;
  //   mean[1]  = 0.0639; error[1] = 0.0096;
  //   mean[2]  = 0.0442; error[2] = 0.0081;
  //   iso = 1;
  // }
  // else {
  //   mean[0]  = 0.0780; error[0] = 0.0212;
  //   mean[1]  = 0.0438; error[1] = 0.0169;
  //   mean[2]  = 0.0269; error[2] = 0.0140;
  //   iso = 2;
  // }
...
int main(int argc, char * argv[]) {
  ...
  // generator level cuts on W mass
  const float WMassLowCut = cfg.get<float>("WMassLowCut");
  const float WMassHighCut = cfg.get<float>("WMassHighCut");
  ...
  TH1D * WMassH     = new TH1D("WMassH","",100,0,1000);
  ...
  Bool_t tauAntiElectronMediumMVA5_;
  ...
  Int_t selection_; // Z->mumu+Jet, W->muv+Jet, 2 : W*->muv, 3 : W*->tauv, 4 : dijet
  ...
  //  ntuple_->Branch("effLoose", &effLoose_, "effLoose/F");
  //  ntuple_->Branch("effMedium",&effMedium_,"effMedium/F");
  //  ntuple_->Branch("effTight", &effTight_, "effTight/F");

  //  ntuple_->Branch("effAntiLLoose", &effAntiLLoose_, "effAntiLLoose/F");
  //  ntuple_->Branch("effAntiLMedium",&effAntiLMedium_,"effAntiLMedium/F");
  //  ntuple_->Branch("effAntiLTight", &effAntiLTight_, "effAntiLTight/F");

  //  ntuple_->Branch("fakeLoose", &fakeLoose_, "fakeLoose/F");
  //  ntuple_->Branch("fakeMedium",&fakeMedium_,"fakeMedium/F");
  //  ntuple_->Branch("fakeTight", &fakeTight_, "fakeTight/F");
  ...
  ntuple_->Branch("tauNTrk",&tauNtrk_,"tauNTrk/i");
  ...
  ntuple_->Branch("tauAntiElectronMediumMVA5",&tauAntiElectronMediumMVA5_,"tauAntiElectronMediumMVA5/O");
  ...
  string fullPathToJsonFile = cmsswBase + "/src/DesyTauAnalyses/NTupleMaker/data/json/" + jsonFile;
  ...
  TFile * filePUOfficial_data = new TFile(TString(cmsswBase)+"/src/DesyTauAnalyses/NTupleMaker/data/PileUpDistrib/Data_Pileup_2015D_Nov17.root","read");
  TFile * filePUOfficial_MC = new TFile (TString(cmsswBase)+"/src//DesyTauAnalyses/NTupleMaker/data/PileUpDistrib/MC_Spring15_PU25_Startup.root", "read");
  ...
  // PU with the number
  TFile * filePUdistribution_data = new TFile(TString(cmsswBase)+"/src/DesyTauAnalyses/NTupleMaker/data/PileUpDistrib/Data_PU.root");
  TFile * filePUdistribution_MC = new TFile (TString(cmsswBase)+"/src/DesyTauAnalyses/NTupleMaker/data/PileUpDistrib/MC_PU.root");
  TH1D * PU_data = (TH1D *)filePUdistribution_data->Get("pileup");
  TH1D * PU_mc = (TH1D *)filePUdistribution_MC->Get("pileup");

  // tau fake rate functions
  TFile * fileTauFakeRate = new TFile(TString(cmsswBase)+"/src/DesyTauAnalyses/NTupleMaker/data/"+tauFakeRateFileName);
  TFile * fileTauAntiLFakeRate = new TFile(TString(cmsswBase)+"/src/DesyTauAnalyses/NTupleMaker/data/"+tauAntiLFakeRateFileName);

  TF1 * tauFakeRateLooseIso = (TF1*)fileTauFakeRate->Get("FitFuncLooseIso");
  TF1 * tauFakeRateMediumIso = (TF1*)fileTauFakeRate->Get("FitFuncMediumIso");
  TF1 * tauFakeRateTightIso = (TF1*)fileTauFakeRate->Get("FitFuncTightIso");

  TF1 * tauFakeRateAntiLLooseIso = (TF1*)fileTauAntiLFakeRate->Get("FitFuncLooseIso");
  TF1 * tauFakeRateAntiLMediumIso = (TF1*)fileTauAntiLFakeRate->Get("FitFuncMediumIso");
  TF1 * tauFakeRateAntiLTightIso = (TF1*)fileTauAntiLFakeRate->Get("FitFuncTightIso");
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) { // loop over files
    ...
    // accessing input events and event weights
    TH1D * histoInputEvents = NULL;
    TH1D * histoEventWeights = NULL;
    if (isData) {
      histoInputEvents = (TH1D*)file_->Get(TString(eventHistoNameData));
      if (histoInputEvents==NULL) {
	cout << "Histogram " << eventHistoNameData << " is not found in file : quitting program" << endl;
	exit(-1);
      }
      inputEventsH->Fill(0.,histoInputEvents->GetBinContent(1));
      //      histWeightsH->Fill(0.,histoInputEvents->GetBinContent(1));
    }
    else {
      histoInputEvents = (TH1D*)file_->Get(TString(eventHistoName));
      histoEventWeights = (TH1D*)file_->Get(TString(weightsHistoName));
      if (histoInputEvents==NULL) { 
	cout << "Histogram " << eventHistoName << " is not found in file : quitting program" << endl;
	exit(-1);
      }
      if (histoEventWeights==NULL) {
        cout << "Histogram " << weightsHistoName << " is not found in file : quitting program" << endl;
        exit(-1);
      }
      inputEventsH->Fill(0.,histoInputEvents->GetBinContent(1));
      //      histWeightsH->Fill(0.,histoEventWeights->GetBinContent(1));
    }

    // accessing tree
    TTree * tree_ = NULL;
    tree_ = (TTree*)file_->Get(TString(ntupleName));
    if (tree_==NULL) { 
    ...
    for (Long64_t iEntry=0; iEntry<numberOfEntries; iEntry++) { 
      ...
      tauAntiElectronMediumMVA5_ = false;
	...
	WTauDecayH->Fill(wTauDecay_,weight_);
      ...
      if (!isData) {
	if (wMass_<WMassLowCut) continue;
	if (wMass_>WMassHighCut) continue;
      }
	...
	// if (analysisTree.primvertex_count>0&&analysisTree.primvertex_count<30) {
	//   double dataPU = PU_data->GetBinContent(PU_data->FindBin(double(analysisTree.primvertex_count)));
	//   double mcPU   = PU_mc->GetBinContent(PU_mc->FindBin(double(analysisTree.primvertex_count)));
	//   puWeight_ = dataPU/mcPU;
	// }
	// else
	//   puWeight_ = 1;
	...
	float absIso = analysisTree.muon_r03_sumChargedHadronPt[imuon];
	...
	tauNtrk_ = analysisTree.tau_ntracks_pt1[indexTau];
	...
	tauAntiElectronMediumMVA5_ = analysisTree.tau_againstElectronMediumMVA5[indexTau] > 0.5;
	...
	if (isWTauNu) {
	  ...
	  fillWNTuple = fillWNTuple && tauDM_>0.5 && tauLooseIso_>0.5 && tauAntiMuonLoose3_>0.5 && tauAntiElectronLooseMVA5_>0.5;
	...
	for (unsigned int iTau=0; iTau<tauIndexes.size(); ++iTau) { // loop over taus
      ...
      if (foundJetTauPair) DiJetEvents++;
