...
//study the origin of mu-tau
	...
	if (analysisTree.tau_pt[it] > ptTauCut &&  
	fabs(analysisTree.tau_eta[it]) < etaTauCut && 
	analysisTree.tau_decayModeFinding[it]>decayModeFinding && 
	fabs(analysisTree.tau_leadchargedhadrcand_dz[it]) < leadchargedhadrcand_dz &&
	fabs(analysisTree.tau_charge[it]) == 1  &&
        analysisTree.tau_byMediumCombinedIsolationDeltaBetaCorr3Hits[it] < 0.5) 
...
//      if ( analysisTree.tau_againstElectronVLooseMVA6[tau_index]>0.5 &&   analysisTree.tau_againstMuonTight3[tau_index]>0.5) MuTauVeto = true;
      if (dRmutau > 0.4) MuTauVeto=true;
      if (q>0) MuTauVeto=true;
	...
	if (relIsoEle>isoVetoElectronCutn) continue;
	...
	if (relIsoMu>isoVetoMuonCutn) continue;
	foundExtraMuon = true;
      }
