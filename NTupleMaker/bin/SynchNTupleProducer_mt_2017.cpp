...
int main(int argc, char * argv[]) {
  ...
  const float ptMuonXTriggerCut = cfg.get<float>("PtMuonXTriggerCut");
  const float etaMuonTriggerCut = cfg.get<float>("EtaMuonTriggerCut");
  ...
  const string MuonLegXTriggerFile = cfg.get<string>("MuonLegXTriggerFile");
  ...
  Float_t         eventtrigweight_1;
  Float_t         eventtrigweight_2;
  Float_t         eventtrigweight_3;
    
  Float_t         eventeffweight_1;
  Float_t         eventeffweight_2;
  Float_t         eventeffweight_3;
  ...
  */
  ...
  tree->Branch("singleL",&singleL,"singleL/O");
  tree->Branch("ltau",&ltau,"ltau/O");
  ...
  tree->Branch("eventeffweight_1", &eventeffweight_1, "eventeffweight_1/F");
  tree->Branch("eventeffweight_2", &eventeffweight_2, "eventeffweight_2/F");
  tree->Branch("eventeffweight_3", &eventeffweight_3, "eventeffweight_3/F");

  tree->Branch("eventtrigweight_1", &eventtrigweight_1, "eventtrigweight_1/F");
  tree->Branch("eventtrigweight_2", &eventtrigweight_2, "eventtrigweight_2/F");
  tree->Branch("eventtrigweight_3", &eventtrigweight_3, "eventtrigweight_3/F");
  ...
  ScaleFactor * SF_muonLegXTrig = new ScaleFactor();
  SF_muonLegXTrig->init_ScaleFactor(TString(cmsswBase)+"/src/"+TString(MuonLegXTriggerFile));
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) {
	...
	recoilMetCorrector.CorrectByMeanResolution(pfmet_ex,pfmet_ey,bosonPx,bosonPy,lepPx,lepPy,njetsforrecoil,pfmet_corr_ex,pfmet_corr_ey);
      ...
      float dPhiMETMu = dPhiFrom2P(analysisTree.muon_px[muonIndex],analysisTree.muon_py[muonIndex],
				   pfmet_ex,pfmet_ey);
      mt_1 = TMath::Sqrt(2*met*pt_1*(1-TMath::Cos(dPhiMETMu)));
	...
	isZLL = isZL || isZJ;
      ...
      isoweight_2 = 1;
      if (gen_match_2==5) isoweight_2 = 0.95;
      ...
      bool isMuonTauOverlapTauMatch = false;
      for (unsigned int iT=0; iT<analysisTree.trigobject_count; ++iT) {
	float dRMuTrig = deltaR(analysisTree.muon_eta[muonIndex],analysisTree.muon_phi[muonIndex],
				analysisTree.trigobject_eta[iT],analysisTree.trigobject_phi[iT]);
	float dRTauTrig = deltaR(analysisTree.tau_eta[tauIndex],analysisTree.tau_phi[tauIndex],
				 analysisTree.trigobject_eta[iT],analysisTree.trigobject_phi[iT]);

	bool etaSingleLTriggerCut = fabs(analysisTree.trigobject_eta[iT]) < etaMuonTriggerCut;
	bool ptMuSingleLTriggerCut = analysisTree.muon_pt[muonIndex] > ptMuonTriggerCut;
	bool ptMuXTriggerCut = analysisTree.muon_pt[muonIndex] > ptMuonXTriggerCut;
	...
	&& isMuonTauOverlapTauMatch && isMuonTauTauLegMatch;
      ...
      float eff_l_data = (float)SF_muonLegXTrig->get_EfficiencyData(double(pt_1),double(eta_1));
      float eff_l_mc = (float)SF_muonLegXTrig->get_EfficiencyMC(double(pt_1),double(eta_1));

      int tau_decayMode = analysisTree.tau_decayMode[tauIndex];

      w_XTrigTauLegSF->var("t_pt")->setVal(pt_2);
      w_XTrigTauLegSF->var("t_eta")->setVal(eta_2);
      w_XTrigTauLegSF->var("t_dm")->setVal(tau_decayMode);
	...
	eff_tau_data = 0;
      ...
      if (analysisTree.muon_pt[muonIndex] < ptMuonTriggerCut) {
	eff_L_mc = 0;
	eff_L_data = 0;
      }
      if (analysisTree.muon_pt[muonIndex] < ptMuonXTriggerCut) {
	eff_l_mc = 0;
	eff_l_data = 0;
      }

      float effDATA = eff_L_data + eff_l_data*eff_tau_data - TMath::Min(float(eff_L_data),float(eff_l_data))*eff_tau_data;
      float effMC = eff_L_mc + eff_l_mc*eff_tau_mc - TMath::Min(float(eff_L_mc),float(eff_l_mc))*eff_tau_mc;

      eventtrigweight_1 = 0;
      if (effMC>0)
	eventtrigweight_1 = effDATA/effMC;


      //      if (analysisTree.muon_pt[muonIndex] < ptMuonTriggerCut) {
      //      std::cout << "Eff_L   = " << eff_L_data << " : " << eff_L_mc << std::endl;
      //      std::cout << "Eff_l   = " << eff_l_data << " : " << eff_l_mc << std::endl;
      //      std::cout << "Eff_tau = " << eff_tau_data << " : " << eff_tau_mc << std::endl;
      //      }      
      ...
      if (L_fired) {
	if (eff_L_mc>0) 
	  eventtrigweight_2 = eff_L_data/eff_L_mc;
      }
      if (!L_fired && X_fired) {
	if (eff_l_mc>0&&eff_tau_mc>0)
	  eventtrigweight_2 = (eff_l_data*eff_tau_data)/(eff_l_mc*eff_tau_mc);
      }

      eventtrigweight_3 = 0;
      if (analysisTree.muon_pt[muonIndex] < ptMuonTriggerCut) {
	if (eff_l_mc>0&&eff_tau_mc>0)
          eventtrigweight_3 = (eff_l_data*eff_tau_data)/(eff_l_mc*eff_tau_mc);
      }
      else {
	if (eff_L_mc>0)
          eventtrigweight_3 = eff_L_data/eff_L_mc;
      }

      if (eventtrigweight_1>10) eventtrigweight_1=10;
      if (eventtrigweight_2>10) eventtrigweight_2=10;
      if (eventtrigweight_3>10) eventtrigweight_3=10;

      singleL = L_fired;
      ltau = X_fired;

      //      if (!L_fired && X_fired) {
      //	std::cout << "L_fired = " << L_fired << "  X_fired = " << X_fired << std::endl;
      //	std::cout << "Weight = " << eventtrigweight_1 << " : " << eventtrigweight_2 << " : " << eventtrigweight_3 << std::endl;
      //	std::cout << std::endl;
      //      }

      effweight = isoweight_1*isoweight_2*eventtrigweight_1;
      eventeffweight_1 = isoweight_1*isoweight_2*eventtrigweight_1;
      eventeffweight_2 = isoweight_1*isoweight_2*eventtrigweight_2;
      eventeffweight_3 = isoweight_1*isoweight_2*eventtrigweight_3;
      ...
      */
      ...
      if (computeDitauMass && mt_1<45.0 && validDecayMode) {
	...
	SVfitStandaloneAlgorithm algo = SVFitMassComputation(svFitMuon, svFitTau,
							     pfmet_ex, pfmet_ey, 
							     covMET, tau_decayMode, inputFile_visPtResolution);
	...
	phi_sv = algo.phi();
      ...
      TLorentzVector metLV(pfmet_ex,pfmet_ey,0.,met);

      // computing total transverse mass
      mTtot = totalTransverseMass         ( muonLV ,     tauLV ,     metLV);
