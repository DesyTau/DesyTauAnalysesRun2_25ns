...
int main(int argc, char * argv[]) {
  ...
  for (int iF=0; iF<nTotalFiles; ++iF) {
	...
	cout << "Filter " << SingleMuonFilterName << " not found " << endl;
        ...
        {
            if(idEles.size() < 1) continue;
            if(idMuons.size() < 3) continue;
        }
        ...
        for(unsigned int im1=0;im1<goodMuons.size();++im1)
        {
            ...
            for(unsigned int im2=im1+1;im2<(goodMuons.size()-im1);++im2)
            {
